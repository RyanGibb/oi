...
(* Project Dockerfile: bake oi + build depexts into a distro stage,
   then [COPY . /src] and [RUN oi build] so the resulting image is a
   reproducible build of the cwd's *.opam project. Driven by
   [oi build --docker] (project mode). *)
let dockerfile_project ?(overlay_depexts = []) d =
  let resolved = Distro.resolve_alias d in
  let tag = Distro.tag_of_distro (resolved :> Distro.t) in
  DF.comment "Generated by `oi build --docker` -- %s project build." tag
