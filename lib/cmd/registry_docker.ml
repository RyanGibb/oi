...
(* S3 bucket the final [s3cmd sync] step writes to. *)
let s3_bucket = "s3://oiu/"
...
let build_sync_shell () =
  Printf.sprintf
    "set -eu; \
     trap 'rm -f /root/.s3cfg' EXIT; \
     printf '[default]\\naccess_key = %%s\\nsecret_key = %%s\\n' \
       \"$(cat /run/secrets/%s)\" \
       \"$(cat /run/secrets/%s)\" > /root/.s3cfg; \
     chmod 600 /root/.s3cfg; \
     OI_BUILD_PARALLELISM=$(nproc) oi build --refresh --all \
       --use-registry=archives --export /registry; \
     s3cmd sync --skip-existing /registry/ %s"
...
let dockerfile_one_distro ?(overlay_depexts = []) d =
  let resolved = Distro.resolve_alias d in
  let tag = Distro.tag_of_distro (resolved :> Distro.t) in
  let cmd = build_sync_shell () in
  DF.comment "Generated by `oi docker --all` -- %s registry build image." tag
  @@ DF.comment
       "Usage: S3_ACCESS_KEY=… S3_SECRET_KEY=… docker compose build \
        (BuildKit secrets carry the keys; the build runs oi build --all \
...
(* docker-compose.yml that drives [oi docker --all]. Each per-distro
   ...
   host. *)
let docker_compose_yaml ~distros () =
  ...
  Printf.ksprintf out
    "# Each per-distro service's image build runs `oi build --refresh --all \
     --export /registry` to populate a per-os_key registry tree, then \
     `s3cmd sync --skip-existing /registry/ %s` to publish it.\n"
    s3_bucket;
  out
...
let obuilder_spec_one_distro ?(overlay_depexts = []) d =
  let resolved = Distro.resolve_alias d in
  let img, tag = Distro.base_distro_tag (resolved :> Distro.t) in
...
|}
    ...
    s3_bucket img tag shell_op (sexp_escape install) (sexp_escape fetch_oi)
    s3_access_key_secret s3_access_key_secret s3_secret_key_secret
    s3_secret_key_secret
    (sexp_escape (build_sync_shell ()))
