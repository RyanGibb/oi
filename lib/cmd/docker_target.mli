(** [oi docker TARGET]: emit a Dockerfile (plus sidecar [recipe.json]) that
    ...
    written out next to the Dockerfile so [oi ir run /work] inside the
    container has the plan to replay. *)
...
(** Source-independent counterpart to {!emit}: emit a Dockerfile that doesn't
    ...
    and the registry index are stable; every other input is resolved at
    build time. Useful when you want a one-file Dockerfile you can hand off
    without a recipe sidecar, accepting that "the build runs at docker-build
    time" rather than "the build replays a pre-computed plan". *)
...
(** [emit ~targets ~distro ~output …] solves [targets] under [distro]'s os_key
    ...
    - [<output>/Dockerfile.oi-<slug>.<distro>] (or that name in the cwd when
      [output] is [None])
    - [<output>/recipe.json] — the d10ir plan the Dockerfile's [oi ir run]
      step consumes.
    ...
    GitHub releases API). [registry] becomes the default [ARG OI_REGISTRY]
    so the archive URLs point at the user's configured mirror. *)
