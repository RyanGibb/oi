(** [oi registry depexts]: print the union of overlay depexts for the host
    ...
    [oi registry build --all] runs — the macOS GitHub-Actions job pipes
    the output through [brew install], paralleling what
    [oi registry docker] bakes into each Linux Dockerfile. *)
