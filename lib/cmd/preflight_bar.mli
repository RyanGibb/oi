...
(** Unified terminal UI for [oi run] / [oi build] / [oi build --all].
    ...
    - {!Theme} — OCaml-themed [Progress] line styling (camel-orange filled bars
      \+ spinners, dim-brown empty bar segments). Use these helpers whenever a
      new [Progress.Line.bar] / [spinner] is constructed so [Pipeline]'s
      pre-built fetch UI and [Execute]'s build UI look visually identical.
...
(** {1 Shared layout}
    ...
    Column budgets used across the pipeline UI. Pin these on every per-row line
    ([Pipeline.fetch_with_display], [Execute.with_default_reporter]) and the
    overall {!Preflight} row so every [│bar│] column lands on the same screen
    column. *)

val row_label_width : int
(** Leading prefix on every row (overall, aggregate, per-layer, per-package).
    Picked to fit the longest [pkg.version] strings we see in practice without
    truncation. *)
...
(** [fit_label n s] pads [s] with spaces to width [n], or truncates with a
    trailing [".."] if [s] is longer. Use for per-row label cells so package
    names line up regardless of length. *)
...
module Theme : sig
  ...
  val bar : width:[ `Fixed of int | `Expand ] -> int -> int Progress.Line.t
  ...
  (** [(N%)] printer: no internal padding, externally lpad'd to width 6 so
      column alignment holds for any 0..100 value. *)
end
...
(** [run ~sw ~clock ~total ~title f] opens an animated single-bar progress
    display, runs [f t], and clears the bar on return. [status_lines] reserves
    rows above the bar for {!log} entries (default 4). Used by [oi build --all];
    new code should reach for {!Preflight} instead. *)
