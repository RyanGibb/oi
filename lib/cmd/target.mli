...
(** [parse_build_target] Classify a raw user-typed token: bare [@handle] becomes
    [Overlay_all], [@handle/pkg…] becomes [Overlay_pkg], anything else is
    [Plain_target]. *)

val bare_handle : string -> string option
(** [bare_handle] when [s] is a bare $(b,@h) handle (no $(b,/pkg) suffix and a
...
(** [split_handle_prefix] for tokens of the form [@handle/pkg…], or [None]
    otherwise. Raises {!Oi.Error.fail_config_error} when [@handle] is given
    without a package. *)
...
(** [extract_handle_pins] Strip [@handle/pkg] prefixes out of a list of tokens.
    Each hit routes [handle] into [with_repos], replaces the token with its
    stripped form, and records a {!handle_pin} so the caller can pin the package
    to the overlay's version. Returns
    [(stripped_tokens, updated_with_repos, handle_pins)]. *)

val pin_handles : handle_pin list -> string list
(** [pin_handles] Project a list of handle-pins down to their handle strings.
    Useful for feeding the [@h/pkg] half of [--with] / [TARGET] tokens into
...
(** [handle_pin_constraints] turns a list of handle-pins into an [= VERSION]
    constraint map suitable for merging into the solver's [constraints]
    argument. A pin without an explicit user constraint is pinned to the highest
    version the overlay ships. [cli_extras] must be fully materialised on disk
    before this runs. *)

(** {1 Extras (--with-repo + project [x-repos:])} *)

val is_url_like : string -> bool
(** [is_url_like] A [--with-repo] / [--with] token is a URL when it has a
    scheme-like prefix or contains a path separator; otherwise it's an overlay
    handle. *)

val handles_of_tokens : string list -> string list
(** [handles_of_tokens] Keep only the handle-shaped tokens (those for which
    {!is_url_like} returns [false]). Used by every solving command to derive the
    handle list fed into {!Oi.Pipeline.pick_toolchain}. *)
...
(** [cli_extra_repos] Resolve a list of [--with-repo] tokens (a mix of URLs and
    reporepo handles) into the flat extra-repo list the solver wants. Later
...
val merge_extras :
  ...
  Oi.Project.extra_repo list
(** [merge_extras] Merge CLI [--with-repo] URLs and project-declared extras.
    Project extras win on a name collision. *)
...
(** [parse_pkg_target] Parse a CLI target as either ["name"], ["name.version"],
    or an opam atom like ["name>=1.0"] / ["name=1.0"]. Returns the bare name and
    an optional version constraint for the solver. *)
