...
let cmd =
  let run () cache_dir =
    Harness.run @@ fun env ->
    let { Harness.proc_mgr = _proc_mgr; fs = _fs; clock = _clock; sys = _sys; platform = _platform; os_key = _os_key; cache = cache } =
...
let cmd =
  let run () cache_dir =
    Harness.run @@ fun env ->
    let { Harness.proc_mgr = _proc_mgr; fs = _fs; clock = _clock; sys = sys; platform = _platform; os_key = _os_key; cache = cache } =
      Harness.bootstrap env cache_dir
    in
