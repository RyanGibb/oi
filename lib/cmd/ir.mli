...
(** Strip [patches:] and [extra-files:] from an opam file in place,
    and remove the sibling [files/] subdirectory holding those blobs.
    Called after a successful bake / restore once [x-d10-archive] is
    set: the consolidated d10ir archive contains the patched +
    extras-included source tree, so the reporepo's copy is dead
    weight. The [files/] dir typically dominates a baked overlay's
    on-disk size. Returns [`Already] when there was nothing to remove,
    [`Stripped] otherwise. *)
