...
(** [ocaml_version] Project-wide default OCaml version (currently [5.4.1]). *)

val app_name : string
(** [app_name] Program name ([oi]). Used by {!Terms} for env-var prefixing and
    by the cmdliner program info. *)
...
(** [resolved_cwd] Canonical absolute path of the current working directory.
    Returns the string form (for env vars and opam APIs that take strings) and
    an [Eio.Path.t] rooted at [fs] (for filesystem operations). [Unix.realpath]
    up front avoids the relative ["."] that [Eio.Stdenv.cwd] would yield via
    [Eio.Path.native_exn]. *)
