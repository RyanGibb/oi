...
(** [parse_build_target] Classify a raw user-typed target token into a
    {!build_target}: bare [@handle] becomes [Overlay_all], [@handle/pkg…]
    becomes [Overlay_pkg], and anything else falls through as [Plain_target]. *)

val bare_handle : string -> string option
(** [bare_handle] when [s] is a bare [@h] handle (no [/pkg] suffix and a
...
(** [split_handle_prefix] for tokens of the form [@handle/pkg…], or [None]
    otherwise. Raises {!Error.fail_config_error} when [@handle] is given without
    a package. *)
