(** Progress events emitted by [lib/oi] subsystems.
    ...
    The library never opens or drives a progress bar itself.
    Functions that have something to report take an optional
    [reporter] and emit typed [event]s; the caller (the [oi]
    cmdliner layer) constructs whatever UI it wants on top.

    The same reporter type covers every phase of an [oi build] /
    [oi run]: solve, registry fetch, source fetch, archive bake,
    layer build, prefix assembly. The cmdliner layer translates
...
(** The kind of asset being fetched. Affects how the UI labels per-row
    progress (binary layer vs. raw source archive). *)
type fetch_kind = Layer | Source

(** Library-emitted progress events. *)
type event =
  | Phase_started of { phase : phase; label : string }
      (** A new phase has begun. [label] is human-readable, often
          identifies the unit being processed (overlay, package). *)
  | Phase_done of phase
      (** A phase ended. The UI may collapse / reset its per-phase
          rows here. *)

  | Status of string
      (** Free-form one-line status text; the UI may render in the
          aggregate row's message column or ignore. *)
  ...
  | Aggregate of {
      ...
      (** Cumulative count for a phase. Sent on each meaningful
          advance so the UI can drive a bar / fraction. *)
  ...
  | Fetch_progress of {
      ...
      total : int64;
          (** Declared total bytes if known (e.g. HTTP Content-Length)
              else [0L]. Lets the UI refine the per-fetch denominator
              when [Fetch_started] couldn't supply a size. *)
    }
  | Fetch_finished of {
      kind : fetch_kind;
      key : string;
  ...
  | Build of D10ir.Direct.event
      (** Per-layer build events propagated from [D10ir.Direct]. The
          UI typically renders these as a per-package row stack. *)

  | Build_summary of D10ir.Direct.result
      (** Final result of the [Building] phase. Lets the UI swap to
          an ok/failed status without polling. *)

type reporter = { event : event -> unit }

val null : reporter
(** No-op reporter: drops every event. Default for callers that
    don't want a UI. *)
