...
let priority = function
  | Outcome.K_build_failed | K_install_failed | K_fetch_failed | K_solve_failed
    ->
      0
  | K_dep_failed | K_depext_missing | K_skipped -> 1
  | K_ok -> 2
  | K_cached | K_restored -> 3

let bump_summary s = function
  | Outcome.K_ok -> { s with ok = s.ok + 1 }
  | K_cached -> { s with cached = s.cached + 1 }
  | K_restored -> { s with restored = s.restored + 1 }
  | K_build_failed -> { s with build_failed = s.build_failed + 1 }
  | K_install_failed -> { s with install_failed = s.install_failed + 1 }
  | K_fetch_failed -> { s with fetch_failed = s.fetch_failed + 1 }
  | K_solve_failed -> { s with solve_failed = s.solve_failed + 1 }
  | K_dep_failed -> { s with dep_failed = s.dep_failed + 1 }
  | K_depext_missing -> { s with depext_missing = s.depext_missing + 1 }
  | K_skipped -> { s with skipped = s.skipped + 1 }
...
let key_of_event (e : Audit.event) : caller_key =
  let overlay =
    Option.map
      (fun (o : D10.Overlay.t) -> (o.handle, o.version))
      e.context.overlay
  in
...
let aggregate_callers (events : Audit.event list) : caller list =
  ...
  List.iter
    (fun (e : Audit.event) ->
      ...
      let kind = Outcome.kind_of e.outcome in
      let updated =
        ...
        | Some c ->
            {
              ...
              count = c.count + 1;
              outcomes = Outcome.bump kind c.outcomes;
            }
      in
      Hashtbl.replace tbl k updated)
    events;
  Hashtbl.fold (fun _ v acc -> v :: acc) tbl []
  |> List.map (fun c -> { c with outcomes = Outcome.sort_histogram c.outcomes })
  |> List.sort (fun a b ->
...
let method_of_outcome (o : Outcome.t) : Identity.method_ =
...
(* Pick the most informative outcome across [events] using [priority]:
   failure outcomes outrank success/cached, and we tie-break by stable
   string form. Caller guarantees [events] is non-empty. *)
let pick_headline events =
  events
  |> List.map (fun (e : Audit.event) -> Outcome.kind_of e.outcome)
  |> List.sort (fun a b ->
...
let from_logs ~os_key ~exported_at provs events =
  (* Bucket [events] by hash. [Solve_key] events live in their own
     ...
     [os_key] is belt-and-braces: callers should already pass a slice
     for the os, but a stray cross-os event would otherwise show up as
     a phantom entry. *)
  let layer_events : (string, Audit.event list) Hashtbl.t =
    Hashtbl.create 256
  in
  let solve_events : (string, Audit.event list) Hashtbl.t = Hashtbl.create 16 in
  let push tbl key v =
    let prev =
  ...
  List.iter
    (fun (e : Audit.event) ->
      if e.os_key = os_key then
        match e.target with
        | Layer h -> push layer_events h e
        | Solve_key h -> push solve_events h e)
    events;
  let take_layer_events hash =
    match Hashtbl.find_opt layer_events hash with
    | Some xs ->
        Hashtbl.remove layer_events hash;
        xs
    | None -> []
  in
  let from_provs =
    List.map
      (fun (p : Provenance.t) ->
        entry_of_provenance p (take_layer_events p.layer_hash))
      provs
  in
  (* Anything left in [layer_events] is a layer-less audit cluster —
     typically build/install failures that never produced a [Provenance.t].
     Solve-failed events live in their own table since they're keyed by
     the solve digest, not a real layer hash. *)
  let from_layer_failures =
    Hashtbl.fold
      (fun _ evs acc -> entry_for_failure_only evs :: acc)
      layer_events []
  in
  let from_solve_failures =
    Hashtbl.fold
      (fun _ evs acc -> entry_for_failure_only evs :: acc)
      solve_events []
  in
  let results = from_provs @ from_layer_failures @ from_solve_failures in
  let summary =
    List.fold_left
...
let outcome_count_codec : (Outcome.kind * int) Jsont.t =
  let open Jsont in
  Object.map ~kind:"outcome_count" (fun kind count -> (kind, count))
  |> Object.mem "kind" Outcome.kind_codec ~enc:(fun (k, _) -> k)
...
let caller_codec : caller Jsont.t =
  ...
  Object.map ~kind:"caller"
    (fun
      overlay toolchain trigger project first_seen last_seen count outcomes ->
      {
        overlay;
        toolchain;
        trigger;
...
let entry_codec : entry Jsont.t =
  ...
  |> Object.mem "pkg" Identity.codec ~enc:(fun (e : entry) -> e.pkg)
  |> Object.mem "os_key" string ~enc:(fun (e : entry) -> e.os_key)
  |> Object.mem "method" Identity.method_codec ~enc:(fun (e : entry) ->
      e.method_)
  |> Object.mem "headline_outcome" Outcome.kind_codec ~enc:(fun (e : entry) ->
      e.headline_outcome)
  |> Object.opt_mem "built_at" number ~enc:(fun (e : entry) -> e.built_at)
  |> Object.opt_mem "duration_s" number ~enc:(fun (e : entry) -> e.duration_s)
  |> Object.opt_mem "phases" Provenance.phases_codec ~enc:(fun (e : entry) ->
      e.phases)
  |> Object.opt_mem "opam" Provenance.opam_info_codec ~enc:(fun (e : entry) ->
      e.opam)
  |> Object.opt_mem "source" Provenance.source_info_codec
  ...
  |> Object.mem "depexts_declared" (list string) ~dec_absent:[]
       ...
       ~enc_omit:(( = ) [])
  |> Object.opt_mem "build_env" Provenance.build_env_codec
       ~enc:(fun (e : entry) -> e.build_env)
  |> Object.mem "callers" (list caller_codec) ~dec_absent:[]
       ~enc:(fun (e : entry) -> e.callers)
       ~enc_omit:(( = ) [])
  |> Object.opt_mem "log" Audit.log_pointer_codec ~enc:(fun (e : entry) ->
      e.log)
  |> Object.finish
