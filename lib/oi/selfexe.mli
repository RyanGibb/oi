...
(** Absolute path to the currently-running [oi] executable, with symlinks
    resolved. Backed by [/proc/self/exe] on Linux (the most reliable source —
    works even when invoked via [PATH] or via a hard / soft link) and by
    [Sys.executable_name] on macOS, which the OCaml runtime populates via
