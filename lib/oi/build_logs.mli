...
(** [<cache_root>/build/logs] — the directory all registry-build log files live
    in. *)

val ensure : fs:Eio.Fs.dir_ty Eio.Path.t -> cache_root:string -> unit
(** Create the logs directory if absent. No-op when it already exists; silent on
...
(** Canonical log file path:
    [<cache_root>/build/logs/<kind>-<name>-<short_hash hash>.log]. [kind] is a
    short tag (e.g. ["build"], ["fetch"], ["solve"]); [name] is the package or
    target; [hash] disambiguates multiple log files that share a name but come
