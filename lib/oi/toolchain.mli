...
(** Fixed-prefix OCaml toolchains.
    ...
    separate namespaces; the latter is what [--toolchain=NAME] resolves
    against.
    ...
    non-relocatable toolchain is built there; subsequent runs skip the
    install. Relocatable toolchains skip the fixed-prefix install entirely. *)
...
type info = {
  ...
  install_prefix : string;
      (** Absolute path where a non-relocatable toolchain lives. Computed
          even for relocatable toolchains (folded into the cache key /
          identity), but the directory is never created on disk in that
          case. *)
  ...
  relocatable : bool;
      (** [true] when the toolchain's compiler can be installed into a
          per-solve consumer prefix — {!ensure_installed} is then a no-op
          and downstream env / [mark_installed] paths skip the fixed-prefix
          treatment. The toolchain still pins the compiler version via
          {!opam_ctx_of_info}. *)
  packages : OpamPackage.Set.t;
      (** Packages installed in the toolchain. *)
  root_names : OpamPackage.Name.Set.t;
      (** Names originally passed as root packages to the toolchain solver
          (e.g. [ocaml-variants], [ocamlfind]). Subset of [packages] by name.
          The consumer solver adds just these as required roots — adding the
          full [packages] set (transitive closure) pulls in oxcaml meta
          packages that trigger unsatisfiable conflicts. *)
  ...
  dep_handles : string list;
      (** Reporepo overlay handles the toolchain layers under (e.g.
          [["default"]]). Surfaced verbatim by [oi show]'s [Repositories]
          block. *)
}

val opam_ctx_of_info : info -> Opam_ctx.toolchain
(** Project a {!info} down to the {!Opam_ctx.toolchain} subset that
    [Opam_ctx.create] / [Solve.solve] / [Prefix.make_env] need. Single source
...
(** [url_of ~handle] returns the URL of the toolchain's primary source — by
    ...
    [depends:] (e.g. [oxcaml] for the oxcaml toolchain). [None] when no
    reporepo entry defines the named toolchain. Used by [oi show] to display
    the toolchain's source repo. *)
...
type summary = {
  ...
  installs : (string * bool) list;
      (** Existing install prefixes on disk for this handle, paired with
          whether each one has its [.oi-toolchain-ready] sentinel. The
          list reflects whatever solves have been cached so far — without
          a solve we can't know the canonical prefix for this machine, so
          callers display all of them. *)
}

val available : unit -> summary list
(** Snapshot of every toolchain definition the reporepo currently advertises
    (entries carrying [x-oi-toolchain-name]) plus any of their install
    prefixes already present under {!default_root}. Cheap (no solve, no
    network), so safe for [oi config]. Empty when the reporepo has no
...
(** [true] when the install_prefix has an [.oi-toolchain-ready] sentinel
    (non-relocatable toolchains) or unconditionally for relocatable ones,
    which need no on-disk preparation. *)

val ensure_installed : fs:Eio.Fs.dir_ty Eio.Path.t -> info -> unit
(** Build the toolchain into its fixed prefix when {!is_ready} is false.
    No-op for relocatable toolchains and for already-prepared
    non-relocatable ones. On failure for a non-relocatable toolchain the
    install_prefix is left partial and {!is_ready} continues to return
    false. *)
